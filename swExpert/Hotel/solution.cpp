...
Heap minPrice[MAXREGION][MAXBEDS][MAXTYPE];
...
void addRoom(int mHotelID, int mRoomID, int mRoomInfo[])
{
    ...
    minPrice[mRoomInfo[0]][mRoomInfo[1]][mRoomInfo[2]].push(mRoomInfo[4], mRoomID, mHotelID);
...
int findRoom(int mFilter[])
{
    ...
    while (minPrice[region][bed][roomType].length > 0 && ans == -1) {
        Data curr = minPrice[region][bed][roomType].pop();
        ...
        if (hotels[curr.hotelID].viewType[idx] != viewType) { continue; }
    ...
    for (int i = 0; i < arrIdx; i++) {
        minPrice[region][bed][roomType].push(arr[i].price, arr[i].roomID, arr[i].hotelID);
    }
...
int riseCosts(int mHotelID)
{
    ...
    for (int i = 0; i < hotels[mHotelID].length; i++) {
        ...
        minPrice[hotels[mHotelID].region[i]][hotels[mHotelID].beds[i]][hotels[mHotelID].roomType[i]].push(
